...
function crear_directorio_documentos($folder, $cod_dis,  $id_inmu) {
    // Validaciones mínimas (evitan volcar 553/000002)
    if (!preg_match('/^\d{2}$/', (string)$cod_dis)) {
        // El distrito debe venir como '01'..'99'
        return false;
    }
    if (!is_numeric($id_inmu)) {
        return false;
    }

    // Base: .../paria/docs
    $directorio_base = rtrim(DOCS_BASE_PATH, "/\\") . '/' . trim($folder, "/\\") . '/docs';

    // Estructura: .../docs/{cod_dis}/{id_inmu_6d}
    $id_inmu_cero = str_pad((string)intval($id_inmu), 6, "0", STR_PAD_LEFT);
    $directorio_distrito = $directorio_base . '/' . $cod_dis;
    $directorio_predio   = $directorio_distrito . '/' . $id_inmu_cero;

    try {
        // Crea toda la ruta si falta
        if (!is_dir($directorio_predio) && !mkdir($directorio_predio, 0755, true)) {
            return false;
        }
