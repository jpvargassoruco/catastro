...
if ((isset($_POST["convalidar"])) and ($_POST["convalidar"] == "REGISTRAR")) {
   ...
   if ($calcular_urbano) {
      ...
      $sql = "INSERT INTO imp_transfer (folio, cod_geo, id_inmu, tit_1id, min_num, not_nom, not_num, not_cls, not_exp, min_val, min_mon, min_fech, id_comp, modo_trans, base_imp, deuda_bs, total_a_pagar, fech_imp, estatus, fech_pago, fech_reg, hora_reg, userid, control, id_comp2)
            VALUES ('$no_orden_conval','$cod_geo','$id_item_temp','$tit_1id','$min_num','$not_nom','$not_num','$not_cls','$not_exp','$min_val','$min_mon','$min_fech','$id_comp','$modo_trans', '$min_val','$cuota_conval','$cuota_conval','$fech_pago_conval','VALIDADO','$fech_pago_conval','$fecha','$hora','$user_id','$control_conval', '$id_comp2')";

   }
   pg_query($sql);
   ### REGISTRAR EL PAGO EN EL BANCO EN IMP_CONTROL_BANCO ###	
   $sql = "INSERT INTO imp_control_banco (fech_pago, nombre_banco, no_boleta_banco, monto_banco, concepto, id_item, gestion, modo_pago, folio, fech_reg, hora_reg, userid_reg) VALUES('$fech_pago_conval','$nombre_banco_conval','$control_conval','$cuota_conval','$concepto_temp','$id_item_temp','$gestion_conval','VALIDADO','$no_orden_conval','$fecha','$hora','$user_id')";
   pg_query($sql);
...
if ((isset($_POST["registro"])) and ($_POST["registro"] == "REGISTRAR") and (!isset($_POST["preliquid_siim"]))) {
   ...
   $nombre_banco_form = $nombre_banco;
   ...
   if ($cuota_tabla == "0") {
      ...
      if (!check_fecha($fecha_banco, $dia_actual, $mes_actual, $ano_actual)) {
         $error_registro = true;
         $mensaje_de_error_registro = "Error: La fecha ingresada no es válida!";
...
if (isset($_POST["estatus"])) {
   ...
   if ($calcular_urbano) {
      ...
      $sql = "SELECT folio FROM imp_transfer WHERE id_inmu = '$id_inmu' AND min_fech = '$min_fech' AND forma_pago = 'CANCELADO'";
   } elseif ($calcular_rural) {
...
if ((!$mostrar_solo_preliquid) and (!$registrar_preliq) and ($estatus != "VALIDADO")) {
   include "igm_impuestos_calcular_monto.php";  
}
...
if (($estatus == "VALIDADO") or ($estatus == "CANCELADO") or ($estatus == "PRESCRIP") or ($mostrar_solo_preliquid)) {
   ...
   if (($calcular_urbano) and (!$mostrar_solo_preliquid)) {
      $sql = "SELECT * FROM imp_pagados WHERE id_inmu = '$id_inmu' AND gestion = '$gestion' AND (forma_pago = 'VALIDADO' OR forma_pago = 'CANCELADO' OR forma_pago = 'PRESCRIP')";
   ...
   if ($estatus == "VALIDADO") {
      ...
      $valor_t = $valor_total = $valor_en_libros_texto = $tp_exen = $base_imp = $cuota_fija = $esc_imp = $base_imp_s_ex = "-";
      $fecha_venc_texto = "-";
   ...
   if ((($estatus == "CANCELADO") or ($estatus == "VALIDADO")) and (($calcular_transfer_urbano) or ($calcular_transfer_rural))) {
      ...
      $fecha_venc_preliquid = "-";
      $nota_exencion = true;
      $texto_exencion = "Se aplicó una rebaja de 100% por motivo de PRESCRIPCION.";
...
if ((isset($_POST["preliquid"])) and ($_POST["preliquid"] == "PRE-LIQUIDACION")) {
   ...
   if ($calcular_urbano) {
      ...
      if ($edi_tipo_total_tabla == "Galpón") {
      ...
      $sql = "INSERT INTO imp_pagados (folio, cod_geo, id_inmu, gestion, forma_pago, tp_inmu, tit_1id, zona, via_mat, 
                        ...
                        credito, saldo, fech_imp, hora_imp, userid_imp, fech_imp_venc, estatus, id_comp)
      ...
      $sql = "INSERT INTO imp_transfer (folio, cod_geo, id_inmu, forma_pago, tit_1id, min_num, not_nom,
            not_num, not_cls, not_exp, min_val, min_mon, min_fech, id_comp, modo_trans, valor_t, valor_vi, valor_total, valor_lib, base_imp, 
            monto_det, fech_venc, descont, exen_id, exencion, monto_imp, dias_venc, ufv_venc, 
            trib_omit, mul_mora, mul_incum, condon_id, condonacion, int_porc, interes, 
            deuda_trib, ufv_actual, deuda_bs, rep_form, pagos_ant, sal_favor, total_a_pagar, sal_prox_gest, 
            credito, saldo, fech_imp, hora_imp, userid_imp, fech_imp_venc, estatus, id_comp2)
            VALUES ('$folio','$cod_geo','$id_inmu','$estatus','$tit_1id','$min_num','$not_nom',
...
################################################################################
#----------- FORMULARIO PARA IMPRIMIR BOLETA DE PAGO IMPUESTOS ----------------#
################################################################################	
if ($boleta_de_pago) {
   echo "<td>\n";
   ...
   if (isset($_POST["id_inmu"])) {
      echo "&nbsp&nbsp <a href=\"index.php?mod=621&inmu=$id_inmu&gestion=$gestion&db=$db&id=$session_id\">\n";
   ...
   echo "siicat_impuestos generar boleta de pagos 1<br>\n";
   ...
   # CUANDO SE PRECIONA EL BOTON DE PRE-LIQUIDACION TAB-2 AL CONTADO
   echo "<td>\n";
   echo "<table border=\"0\" align=\"center\" cellpadding=\"0\" width=\"840px\" height=\"1100px\">\n";   # 3 Columnas
   echo "<tr height=\"40px\">\n";
   echo "<td width=\"15%\">\n";
   ...
   if ($calcular_urbano) {
      if ($imprimir_boleta_de_pago) {
         ...
         echo "Pre-Liquidación IPBI\n";
      }
   } elseif ($calcular_transfer_urbano) {
      if ($imprimir_boleta_de_pago) {
         echo "Boleta de pago Transferencia Urbana\n";
      } else {
         echo "Pre-Liquidación Transferencia Urbana\n";
      }
   } elseif ($calcular_rural) {
      if ($imprimir_boleta_de_pago) {
         echo "Boleta de pago IPA\n";
      } else {
         echo "Pre-Liquidación IPA\n";
      }
   } elseif ($calcular_transfer_rural) {
      if ($imprimir_boleta_de_pago) {
         echo "Boleta de pago Transferencia Rural\n";
      } else {
         echo "Pre-Liquidación Transferencia Rural\n";
      }
   } elseif ($calcular_patente) {
      if ($imprimir_boleta_de_pago) {
         echo "Boleta de pago Impuestos a los Patentes\n";
      } else {
         echo "Pre-Liquidación Impuestos a los Patentes\n";
      }
   }
   echo "</td>\n";
   echo "<td width=\"25%\"> &nbsp</td>\n";   #Col. 3 			 
   ...
   if ($calcular_urbano) {
      ...
      if (($form_trans == 1) or (!$imprimir_boleta_de_pago)) {
         ...
         echo "<iframe frameborder=\"1\" name=\"mapserver\" src=\"http://$server/tmp/preliq$cod_cat.html\" id=\"mapserver\" width=\"840px\" height=\"550px\" align=\"left\" scrolling=\"no\"  noresize=\"no\" marginwidth=\"0\" marginheight=\"0\">\n";
         echo "<br> IMPUESTOS GENERAR PRELIQUIDACION TRANFERENCIA 1";
      } elseif ($form_trans == 2) {
         include "siicat_impuestos_generar_preliquid_transfer2.php";
         echo "<iframe frameborder=\"1\" name=\"mapserver\" src=\"http://$server/tmp/preliq$cod_cat.html\" id=\"mapserver\" width=\"840px\" height=\"1100px\" align=\"left\" scrolling=\"no\"  noresize=\"no\" marginwidth=\"0\" marginheight=\"0\">\n";
         echo "<br> IMPUESTOS GENERAR PRELIQUIDACION TRANFERENCIA 2";
      } else {
         include "siicat_impuestos_generar_preliquid_transfer3.php";
         echo "<iframe frameborder=\"1\" name=\"mapserver\" src=\"http://$server/tmp/boleta_trans_$cod_cat.html\" id=\"mapserver\" width=\"840px\" height=\"1200px\" align=\"left\" scrolling=\"no\"  noresize=\"no\" marginwidth=\"0\" marginheight=\"0\">\n";
         echo "<br> IMPUESTOS GENERAR PRELIQUIDACION TRANFERENCIA 3";
   ...
   echo "<td align=\"center\" valign=\"center\" height=\"40\" width=\"60%\" class=\"pageName\">\n";
   if ($calcular_urbano) {
      echo "Liquidación de Impuestos IPBI\n";
   } elseif ($calcular_rural) {
      echo "Liquidación de IPA\n";
   } elseif ($calcular_patente) {
      echo "Liquidación de Patentes\n";
   } elseif (($calcular_transfer_urbano) or ($calcular_transfer_rural)) {
   ...
   if (!$mostrar_solo_preliquid) {
      ...
      if ($calcular_urbano) {
         echo "<b>Codigo Inmueble: $cod_cat &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Gestión: $gestion &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Impuesto Neto: $imp_neto_post Bs.</b>\n";
      } elseif ($calcular_transfer_urbano) {
         echo "<b>Codigo Inmueble: $cod_cat &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Impuesto a la Transferencia1: $imp_neto_post Bs.</b>\n";
      } elseif ($calcular_rural) {
         echo "<b>Codigo Propiedad: $cod_cat &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Gestión: $gestion &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Impuesto Neto: $imp_neto_post Bs.</b>\n";
      } elseif ($calcular_transfer_rural) {
         echo "<b>Codigo Propiedad: $cod_cat &nbsp&nbsp&nbsp&nbsp&nbsp&nbsp Impuesto a la Transferencia2: $imp_neto_post Bs.</b>\n";
      }
      echo "</td>\n";
      echo "<td>&nbsp</td>\n";   #Col. 3	 		 
      echo "</tr>\n";
   ...
   echo "</ul>\n";
   echo "<div id=\"tab-1\">\n";
   # BOLETA DE PAGO PRIMER TAB
   include "igm_impuestos_detalle_de_pago.php";
   echo "</div>\n";
   if (($estatus != "VALIDADO") and ($estatus != "CANCELADO") and ($estatus != "PRESCRIP") and (!$mostrar_solo_preliquid)) {
      echo "<div id=\"tab-2\">\n";
      include "igm_impuestos_al_contado_transf.php";
      echo "</div>\n";
   }
   if (($estatus != "VALIDADO") and ($estatus != "CANCELADO") and ($estatus != "PRESCRIP") and (!$registro_banco_existe) and (!$mostrar_solo_preliquid) and ($nivel > 1)) {
      echo "<div id=\"tab-3\">\n";
