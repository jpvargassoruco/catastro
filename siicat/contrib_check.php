...
if (($mod == 122) and ((isset($_POST["submit"])) and (($_POST["submit"] == "Registrar")))) {
	...
	$sql2 = "SELECT id_contrib FROM contribuyentes WHERE con_pat = '$con_pat' AND con_mat = '$con_mat' AND con_nom1 = '$con_nom1' AND con_nom2 = '$con_nom2' AND con_tipo = 'PER' AND doc_num = '$doc_num'";
	...
	if (($con_tipo == "PER") and ($con_pat == "")) {
		...
		$mensaje_de_error2 = "Error: Ya existe un contribuyente con el ese nombre en la base de datos!";
	...
	} elseif (pg_num_rows(pg_query($sql3)) > 0) {
		$mensaje_de_error2 = "Error: el nombre y los apellidos ya existen en la base de datos!";
	} else {
		$error2 = false;
	}
	...
	$error3 = true;
	$sql = "SELECT id_contrib FROM contribuyentes WHERE doc_tipo = '$doc_tipo' AND doc_num = '$doc_num' AND doc_exp = '$doc_exp'";
	...
	if ((!$error1) and (!$error2) and (!$error3) and (!$error4)) {
		...
		if ($_POST["submit"] == "Registrar") {
			...
			$sql_contrib = "INSERT INTO contribuyentes 
					...
					dom_bar,dom_tipo,dom_nom,dom_num,dom_edif,dom_bloq,dom_piso,dom_apto,con_obs,tit_pers) 
					VALUES
					('$id_contrib','$con_pmc','$pmc_ant','$con_act','$con_fech_ini','$con_tipo','$con_raz','$con_pat','$con_mat',
							'$con_nom1','$con_nom2','$con_nit','$con_tel','$doc_tipo','$doc_num','$doc_exp','$con_fecnac','$dom_dpto','$dom_ciu', 
					'$dom_bar','$dom_tipo','$dom_nom','$dom_num','$dom_edif','$dom_bloq','$dom_piso','$dom_apto','$con_obs','$tit_pers')";
...
if (($accion == "Registrar") or  ($error1) or ($error2) or ($error3) or ($error4) or ($error5)) {
	include "contrib_formulario.php";
} else {
