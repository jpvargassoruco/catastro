...
if ((isset($_POST["id_inmu"])) OR (isset($_GET["inmu"]))) {
   ...
   if (isset($_POST["id_predio_rural"])) {
	    $id_item = $id_predio_rural = $_POST["id_predio_rural"];
	 } else {	
	    $id_item = $id_predio_rural = $_GET["id_predio_rural"];
	 }
   $calcular_transfer_rural = true;
	 $concepto = "TRANSFER RURAL";
	 $id_item = $id_predio_rural;
	 $tabla_transfer = "imp_transfer_rural";
	 $tabla_contrib_add = "info_predio_rural_contrib_add";
	 $where = "id_predio_rural = '$id_predio_rural'";
	 $accion_reg = "Transfer Rural";
   ### LEER DATOS ANTERIORES ###	
...
########################################	
#$sql="SELECT tan_fech_ini FROM transfer WHERE cod_geo = '$cod_geo' AND id_inmu = '$id_inmu' ORDER BY tan_fech_ini DESC LIMIT 1";
#$check = pg_num_rows(pg_query($sql));
#if ($check > 0) {
#   $result = pg_query($sql);
#   $info = pg_fetch_array($result, null, PGSQL_ASSOC);	
#	 $adq_fech_ultimo = $info['tan_fech_ini'];
#   pg_free_result($result);
#} else $adq_fech_ultimo = "1900-01-01";
######################################
...
if ((isset($_POST["convalidar"])) AND ($_POST["convalidar"] == "REGISTRAR")) {
   ...
   $folio_select = $no_orden_conval = (int) $_POST["no_orden"];
   ...
   ######################################		
   $sql="SELECT min_num, id_comp, modo_trans, min_val, min_mon, min_fech, total_a_pagar, fech_imp, fech_imp_venc FROM $tabla_transfer WHERE folio = '$folio_select'";	 	
   $result = pg_query($sql);
   $info = pg_fetch_array($result, null, PGSQL_ASSOC);
...
#echo "REGISTRO ES $error_registro<br />";
}
########################################
#---------- RELLENAR TABLAS -----------#
########################################		
   ...
   ######################################
	 if ($registro_imp_control_banco) {			
      ...
      $sql = "INSERT INTO imp_control_banco (fech_pago, nombre_banco, no_boleta_banco, monto_banco, concepto, id_item, gestion, modo_pago, folio, fech_reg, hora_reg, userid_reg)
              VALUES('$fecha_banco_ymd','$nombre_banco','$no_boleta_banco','$cuota_tabla','$concepto','$id_inmu','$gestion','CONTADO','$folio_select','$fecha','$hora','$user_id')"; 
#echo "CHECK TRANSFER L211 SQL: $sql<br />";
      pg_query($sql);
	 }	
   ######################################
...
#echo "L325 SQL: $sql<br />";
      ...
      if ((isset($_POST["convalidar"])) AND ($_POST["convalidar"] == "REGISTRAR")) {
	          ...
	          $sql = "INSERT INTO imp_transfer (folio, cod_geo, id_inmu, tit_1id, min_num, not_nom,
				         ...
				         VALUES ('$no_orden_conval','$cod_geo','$id_inmu','$tit_1id','$min_num','$not_nom',
								 '$not_num','$not_cls','$not_exp','$min_val','$min_mon','$min_fech','$id_comp','$modo_trans',
...
#echo "CHECK TRANSFER L249 SQL: $sql<br />";
      pg_query($sql);				 
...
#echo "L335 SQL: $sql<br />";
			   ...
			   $sql = "UPDATE imp_transfer_rural SET estatus = 'OBSOLETO' WHERE folio != '$folio_select' AND id_predio_rural = '$id_predio_rural' AND estatus = 'PRELIQUID'";	
      }
			pg_query($sql); 
      ### LLENAR TABLA TRANSFER ###
			$adq_doc_ant = utf8_encode($adq_doc_ant);
			if ($calcular_transfer_urbano) {
	       $sql = "INSERT INTO transfer (cod_geo, id_inmu, id_proc, tan_fech_ini, tan_fech_fin, 
	           tan_modo, tan_doc, tan_mont_usd, tan_mont_bs, tan_cara, 
						 tan_1id, tan_2id, tan_der_fech, tan_der_num, tan_folio) 
		         VALUES ('$cod_geo','$id_inmu','0','$adq_fech_ant_temp','$min_fech', 
						 '$adq_modo_ant','$adq_doc_ant','$adq_mont_usd_ant','$adq_mont_bs_ant','$tit_cara_ant',
             '$tit_1id_ant','$tit_2id_ant','$der_fech_ant','$der_num_ant','$folio_select')";
      } elseif ($calcular_transfer_rural) {
				 $tp_inmu = "PROPIEDAD RURAL";
				 $tp_viv_texto = "PROPIEDAD RURAL";
				 $sup_const = $deprec = 0;
	       $sql = "INSERT INTO transfer_rural (id_predio_rural, id_proc, tan_fech_ini, tan_fech_fin, 
	           tan_modo, tan_doc, tan_mont_usd, tan_mont_bs, tan_cara, 
						 tan_1id, tan_2id, tan_der_fech, tan_der_num, tan_folio) 
		         VALUES ('$id_predio_rural','0','$adq_fech_ant_temp','$min_fech', 
						 '$adq_modo_ant','$adq_doc_ant','$adq_mont_usd_ant','$adq_mont_bs_ant','$tit_cara_ant',
             '$tit_1id_ant','$tit_2id_ant','$der_fech_ant','$der_num_ant','$folio_select')";
			}
#echo "CHECK TRANSFER L279 $sql<br />";			
      pg_query($sql);
...
#echo "L354 $sql<br />";				        
      ...
      include "c:/apache2/siicat/siicat_impuestos_boleta_de_pago.php";		      	  
} else {
   include "c:/apache2/siicat/siicat_transferencia.php";
}
