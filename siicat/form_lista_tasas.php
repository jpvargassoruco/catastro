<?php
	...
	$sql="SELECT id_tasa, descrip, monto, unidad, costo, id_tra, tipo, rubro FROM tasas ORDER BY id_tasa";
	$result=pg_query($sql);			
	$i = $j = 0;	 
	while ($line = pg_fetch_array($result, null, PGSQL_ASSOC)) {
		foreach ($line as $col_value) {	
			if ($i == 0) {
				...
				$id_tra_lista[$j] = $col_value;
			} elseif ($i == 6) {
				$tipo_lista[$j] = $col_value;																												
			} else {
				$rubro_lista[$j] = $col_value;
				$no_rubro = $rubro_lista[$j];
				...
				$descrip_lista[$j] = $tipo_lista[$j]." &nbsp ".$descrip_lista[$j]." &nbsp ".$costo_lista[$j] ;										 
				$i = -1;
			}
			$i++;
		}
