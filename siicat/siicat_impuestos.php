...
$mostrar = false;
$resultado = false;
$error = $error_fecha_venc = $error_tapr = $error_zona = $error_fact_incl = $error_esc_imp = $aviso = $error_ubi = false;
$predio_existe = false;
$mensaje_de_error = "";
################################################################################
#------------------------------ BOTON CALCULAR --------------------------------#
...
if ((isset($_POST["Registrar"])) AND ($_POST["Registrar"] == "REGISTRAR")) {
   ...
   if (isset($_POST["exen_id"])) { 	
      ...
      pg_query("UPDATE imp_pagados 
      	       SET forma_pago = 'CONTADO', d10 = '0', mant_val = '0', cuota = '0', interes = '0', deb_for = '0', por_form = '0',  monto = '0', descont = '$descont', exen_id = '$exen_id', fech_imp = '$fecha', hora = '$hora', usuario = '$user_id', control = '00000000', no_orden = '1'
...
if ((isset($_POST["Rectificar"])) AND ($_POST["Rectificar"] == "SI")) {	
	...
	WHERE no_orden = '$no_orden_rect' AND gestion = '$gestion_rect' AND cod_geo = '$cod_geo' AND id_inmu = '$id_inmu'";
	pg_query($sql); 	 
}
################################################################################
...
################################################################################	
$sql="SELECT cod_uv FROM info_edif WHERE cod_geo = '$cod_geo' AND cod_uv = '$cod_uv' AND cod_man = '$cod_man' AND cod_pred = '$cod_pred'  AND edi_ano <= '$gestion[$j]";
...
################################################################################
#------------------ SACAR NOMBRE Y APELLIDO CONTRIBUYENTE 1 -------------------#
################################################################################
$sql="SELECT con_pat, con_mat, con_nom1, pmc_ant, doc_num, dom_nom FROM contribuyentes WHERE id_contrib = '$tit_1id'";
...
#echo "EDIF: $edi_area, $sup_const_sat[$k], $delta_sup_const[$k]<br>";								 								 								 		
      ...
      ########################################				
			...
			if (($cod_pad != "") AND ($cod_pad_ext <= $no_de_reg_satinmus)) { 
   ...
   if ($check_satliqin > 0) {
      ...
      while ($line = pg_fetch_array($result_siim_satliqin, null, PGSQL_ASSOC)) {
         foreach ($line as $col_value) {
			...
			$forma_pago[$j] = $valor_por_m2_terr[$j] = $factores_terreno[$j] = $avaluo_terr[$j] = 0;
			$calidad_const[$j] = $avaluo_const[$j] = $factor_deprec[$j] = $avaluo_total[$j] = 0;
			$monto_exen[$j] = $cuota_fija[$j] = $tp_exen[$j] = $base_imp[$j] = $ufv_venc[$j] = 0;
			$mant_valor[$j] = $interes[$j] = $monto[$j] = $d10[$j] = 0;
			$j++;
			$no_de_registros++;
      }
...
$sql="SELECT gestion, forma_pago, tp_inmu, zona, sup_terr, sup_const, fd_an, imp_neto, fech_venc, fech_imp, monto, cuota, no_orden 
      FROM imp_pagados 
	  WHERE cod_geo = '$cod_geo' AND id_inmu = '$id_inmu' AND forma_pago != '' 
...
if ($check_imp_pag > 0) {
	...
	while ($line = pg_fetch_array($result, null, PGSQL_ASSOC)) {
		...
		$valor_por_m2_terr[$j] = $factores_terreno[$j] = $avaluo_terr[$j] = 0;
		$calidad_const[$j] = $ant_const[$j] = $avaluo_const[$j] = $avaluo_total[$j] = 0;
		$monto_exen[$j] = $cuota_fija[$j] = $tp_exen[$j] = $base_imp[$j] = $ufv_venc[$j] = 0;
		$mant_valor[$j] = $interes[$j] = $d10[$j] = 0;			
		$j++;
		$no_de_registros++;
	}
...
while ($ultimo_ano_pagado < $ano_actual-1) {
   ...
   ###########################################################
   #------- AREA PREDIO SOLO PARA PUERTO VILLARROEL ---------#
   ###########################################################
	if ($usa_seg_men){
		$sup_terr[$j] = $ter_smen;
	} else $sup_terr[$j] = $ter_sdoc;
   ###############################
   #----- AREA EDIFICACIONES ----#
   ###############################
   ...
   if ($fact_incl == -1) {
      ...
      $fact_incl_color = "red";
      $mensaje_de_error_fact_incl = "Error: Tiene que especificar la inclinación del terreno!";
   }
   ################################
   #----- FACTOR DE FORMA K3 -----#
   ################################
   ...
   ####################################
   #----- FACTOR DE UBICACION K4 -----#
   ####################################
	if ($ter_ubi==""){
		$fact_ubi_color = "red";
		$error_ubi = true;
		$mensaje_de_error_ubi = "ERROR: No hay datos en ubicacion del terreno, comunicar a la seccion catastro!";	
	}else{
	if ($ter_ubi == "CEN"){
		$fact_ubi = 1;
	} elseif ($ter_ubi == "ESQ") {
   ...
   if ($no_de_edificaciones == 0) {
      ...
      while ($line = pg_fetch_array($result_edif, null, PGSQL_ASSOC)) {	
         ...
         ##echo "$k , $info_valua ,  $line_media[$i] , $line_value[$i] , $no_de_objetos_validos[$i] <br>";
	      $k = 0;
         $i++;	
      } # FIN DE WHILE	
      ########################################
...
#echo "GESTION: $gestion[$j], FECHA VENC: $fecha_venc[$j], UFV VENC: $ufv_venc[$j], FECHA ACTUAL: $fecha, UFV ACTUAL: $ufv_actual<br>";					 
         $mant_valor[$j] = calc_mant_valor($ufv_venc[$j],$ufv_actual,$imp_neto[$j]);
				...
				#############################################################################
				#SOLO POR UN TIEMPO SE PONE EN CERO LOS INTERESES MULTAS POR INCUMPLIMIENTO
				#############################################################################
	...
	echo "</tr>\n";
	 ...
	 while ($i < $no_de_registros) {
	    ...
	    echo "<tr>\n";			 
 	    ...
 	    echo "<td align=\"center\" class=\"bodyTextD\">$sup_terr[$j] m2</td>\n";
 	    echo "<td align=\"center\" class=\"bodyTextD\">$sup_const[$j] m2</td>\n";	 
		...
		if ($sistema[$j] == "SIIM") {			
 	       ...
 	       echo "<input name=\"id_inmu_siim\" type=\"hidden\" value=\"$id_inmu_siim[$j]\">\n";
		}						
		if ((($fecha_pagado[$j] == "BOLETA") OR ($fecha_pagado[$j] == "-")) AND ($activo == 1) AND ($predio_existe)) {
			if ($cuota[$j] != "-") {	       
				echo "<input type=\"submit\" name=\"boleta\" class=\"smallText\" style=\"color: #21618c\" value=\"Aviso\">\n";
			}
		} elseif (($fecha_pagado[$j] == "PLAN") AND ($activo == 1) AND ($predio_existe)) { 
			echo "&nbsp\n";	
		} elseif (($activo == 1) AND ($predio_existe)) {	 
			echo "<input type=\"submit\" name=\"boleta\" class=\"smallText\" style=\"color: #1e8449\" value=\"Boleta\">\n";		
		}	else {
		echo "-\n";				
		}		       
	    echo "</td>\n";	
...
echo "</table>\n";
	...
	if ($error_ubi) {	
		echo "<tr>\n";
		echo "<td align=\"center\" colspan=\"3\" class=\"bodyTextD\">\n";  #Col. 1	
		echo "<font color=\"$fact_ubi_color \"> $mensaje_de_error_ubi</font>\n";
		echo "</td>\n";   
		echo "</tr>\n";
	}
